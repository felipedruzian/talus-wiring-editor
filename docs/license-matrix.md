...
Exigida pelo critério de aceite da issue #1 antes de qualquer novo reuso: um
...
| `Garth-42/WireForm` | `github.com/Garth-42/WireForm` | `5658987` | **GPL-3.0** | **Não — não incorporado.** | Licença e existência do commit confirmadas via `gh api repos/Garth-42/WireForm` em 2026-08-27. A importação WireViz desta fatia (`wireviz-import/*`) é uma implementação **clean-room**: parser de subconjunto YAML e validador de subconjunto WireViz escritos à mão, cobrindo apenas o fixture mínimo de 2 nets que esta issue precisa. Nenhum código-fonte, teste ou fixture do `WireForm` foi lido, copiado ou adaptado durante a escrita — esta é uma declaração sobre a prática de desenvolvimento adotada nesta fatia, não algo verificável externamente. Por restrição explícita desta tarefa, **nenhum código GPL-3.0 está presente neste repositório nesta fatia.** A decisão de licença consolidada do fork (GPL-3.0, *se e quando* código do `WireForm` for de fato incorporado) não se aplica ainda — esta fatia é publicada sem acionar essa mudança. |
| WireViz (o formato/ferramenta) | `github.com/wireviz/WireViz` | n/a — apenas um subconjunto da estrutura YAML de `connectors`/`cables`/`connections` foi usado como contrato de interoperabilidade | GPL-3.0 (a ferramenta WireViz em si) | **Nenhum código usado — apenas compatibilidade de formato.** | Licença confirmada via `gh api repos/wireviz/WireViz` em 2026-08-27. O fixture e o parser desta fatia foram escritos de forma independente para interoperar com esse formato; nenhum código Python do WireViz foi copiado ou transcrito. Esta descrição técnica não substitui análise jurídica. |
